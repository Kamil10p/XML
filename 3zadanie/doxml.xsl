...
<xsl:variable name="id_pacjenta" select="@numer_id" />
    <karta_pacjenta numer_id="{@numer_id}" plec="{dane_personalne/@płeć}">
	...
	<dane_personalne>
		...
		<imie><xsl:value-of select="dane_personalne/imię"/></imie>
	</dane_personalne>
    ...
    <ile_mezczyzn><xsl:value-of select="count(//przychodnia/baza_danych/karty_pacjentów/karta_pacjenta/dane_personalne/@płeć[.= 'M'])"/></ile_mezczyzn>
